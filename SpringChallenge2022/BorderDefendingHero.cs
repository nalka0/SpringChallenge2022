...
namespace SpringChallenge2022
{
    public class BorderDefendingHero : FriendlyHero
    {
                ...
                while (i < Game.PredictedTurns)
                {
                    ...
                    if ((possibleTargets.Count > 0 && Game.PredictedTurns - 1 == i) || defenseNeeded)
                    {
                        ...
                        else
                        {
                            ...
                            if (Game.Me.Mana >= 30 && ShieldDuration <= 1 && Game.Ennemy.Heroes.Any(x => Helpers.IsInRange(x.Position, Position, 2200)))
                            {
                                return Shield(this);
                            }
                            else if (Game.Me.Mana >= 20 && Game.Me.Heroes.FirstOrDefault(friend => friend.ShieldDuration <= 1 && friend.IsControlled && Helpers.IsInRange(friend.Position, Position, 2200) && Game.Ennemy.Heroes.Any(x => Helpers.IsInRange(friend.Position, x.Position, 2200))) is FriendlyHero saved)
                            {
                                return Shield(saved);
                            }
        ...
        {
            if (isDefending && ShieldDuration <= 1 && Game.Me.Mana >= 18 && Game.Ennemy.Heroes.Any(x => Helpers.IsInRange(x.Position, Position, 2200)))
            {
                return Shield(this);
            }
